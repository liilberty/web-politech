document.addEventListener('DOMContentLoaded', () => {
    fetch("foods.json").then(response => response.json()).then(data => {//выборка и сортировка данных
        const sortedFood = data['dishes'].sort((a, b) => {
            return a['name'].localeCompare(b['name'], 'ru');
        });
//привязка к соответствующим контейнерам на веб-странице, чтобы в них динамически добавлять элементы 
...
//отображения блюд по категориям
...
//Настраивает фильтры для каждой категории 
...
//Работа с кнопкой отправки формы и уведомлениями
